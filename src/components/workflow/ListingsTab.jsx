...
const ListingsTab = ({
  ...
  const qaQueue = useWorkflowStore(state => state.qaQueue || []);
  const qaThreshold = useWorkflowStore(state => state.qaThreshold);
  const approveQAItem = useWorkflowStore(state => state.approveQAItem);
  const removeFromQAQueue = useWorkflowStore(state => state.removeFromQAQueue);
          ...
          <Stack spacing={3}>
            ...
            <Card variant="outlined" sx={{ mt: 3 }}>
              <CardContent>
                <Stack direction="row" alignItems="center" spacing={2} mb={2}>
                  <Error color="warning" />
                  <Typography variant="h6">QA Queue ({qaQueue.length})</Typography>
                  <Typography variant="caption" color="text.secondary">Threshold: {Math.round((qaThreshold||0.85)*100)}%</Typography>
                </Stack>

                {qaQueue.length === 0 ? (
                  <Alert severity="success">No items awaiting review. All good!</Alert>
                ) : (
                  <Grid container spacing={2}>
                    {qaQueue.map((q) => (
                      <Grid item xs={12} sm={6} md={4} key={q.imageId}>
                        <Card variant="outlined">
                          <CardContent>
                            <img src={q.processedImage.processed} alt={q.processedImage.title} style={{ width: '100%', height: 120, objectFit: 'cover', borderRadius: 4, marginBottom: 8 }} />
                            <Typography variant="subtitle2" sx={{ mb: 1 }}>{q.processedImage.title}</Typography>
                            <Typography variant="caption" color="text.secondary" display="block">Score: {q.score.toFixed(2)}</Typography>
                            {q.issues && q.issues.length > 0 && (
                              <Alert severity="warning" sx={{ mt: 1 }}>
                                {q.issues.map((iss, idx) => <div key={idx}>{iss.reason}</div>)}
                              </Alert>
                            )}

                            <Stack direction="row" spacing={1} justifyContent="space-between" sx={{ mt: 1 }}>
                              <Button size="small" variant="contained" onClick={() => { approveQAItem(q.imageId); }}>Approve</Button>
                              <Button size="small" variant="outlined" color="error" onClick={() => removeFromQAQueue(q.imageId)}>Reject</Button>
                            </Stack>
                          </CardContent>
                        </Card>
                      </Grid>
